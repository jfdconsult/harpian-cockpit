...
function FaixaDefesa({
  frac, dates, largura = 1000, altura = 34, onHover, marcado,
}: {
...
function CurvaCapital({
  ...
  useEffect(() => {
    ...
    const chart: IChartApi = createChart(ref.current, {
      ...
      rightPriceScale: { borderColor: "#16304f", mode: 1 },
  ...
  return (
    <div style={{ padding: "18px 20px 40px", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(300px, 380px) 1fr", gap: 16, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {/* Descrição do SET carregado. Sem isto o cliente vê três sleeves com
              nomes técnicos e não sabe de quem é o quê — e a atribuição
              AlphaDroid × Harpian é contrato, não detalhe de copy. */}
          {setAtivo && setsData && (() => {
            const d = presetsVitrine().find((p) => p.id === setAtivo)!.def;
            return (
              <section style={{
                background: "var(--panel)", border: "1px solid var(--line)",
                borderLeft: "3px solid var(--gold)", borderRadius: "var(--r-md)",
                padding: "13px 16px 15px", display: "flex", flexDirection: "column", gap: 10,
              }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
                  <span style={{
                    fontFamily: "var(--mono)", fontSize: 9.5, fontWeight: 700, letterSpacing: ".08em",
                    padding: "2px 5px", borderRadius: 3, background: "var(--gold)", color: "#0b1220",
                  }}>SET</span>
                  <h2 style={{ margin: 0, fontSize: 16, fontWeight: 650, letterSpacing: "-.01em" }}>{d.nome}</h2>
                  <span style={{ fontSize: 12, color: "var(--tx3)" }}>
                    {d.volTarget
                      ? `com alvo de volatilidade de ${(d.volTarget.alvo * 100).toFixed(0)}% ao ano`
                      : "sempre 100% investido"}
                  </span>
                </div>
                {/* A tese e a promessa em uma linha. O detalhe de COMO a rotacao
                    escolhe saiu daqui: agora mora na explicacao do proprio bloco,
                    onde o cliente esta olhando quando pergunta "isso e o que?". */}
                <p style={{ margin: 0, fontSize: 13, color: "var(--tx2)", maxWidth: "84ch", lineHeight: 1.5 }}>
                  {d.tese} Nada foi escolhido olhando para trás: a carteira de cada mês foi decidida
                  com a informação daquele mês. Rebalance no primeiro pregão, decidido no fechamento e
                  valendo do dia seguinte.
                </p>
                {/* Cada bloco com o que ele FAZ, nao so o rotulo. "Rotacao por
                    momento" nao diz nada para quem esta do outro lado da mesa —
                    e "rotacao", sozinha, convida a leitura errada de que se sai
                    de acao para renda fixa. Rotaciona ENTRE ESTRATEGIAS. */}
                <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                  {descricaoDoSet(d).map((c) => (
                    <div key={c.bloco} style={{ display: "flex", gap: 10, alignItems: "baseline" }}>
                      <b style={{
                        fontFamily: "var(--mono)", color: "var(--tx1)", fontSize: 13,
                        minWidth: 34, textAlign: "right", flexShrink: 0,
                      }}>{(c.peso * 100).toFixed(0)}%</b>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: 12.5, color: "var(--tx1)" }}>
                          {c.nome}{" "}
                          <span style={{ color: "var(--tx3)", fontSize: 12 }}>— {c.atribuicao}</span>
                        </div>
                        <div style={{
                          fontSize: 12, color: "var(--tx2)", lineHeight: 1.45,
                          marginTop: 1, maxWidth: "80ch",
                        }}>{c.explicacao}</div>
                      </div>
                    </div>
                  ))}

                  {/* A UNICA diferenca entre este SET e o par sem overlay. Ela
                      nao aparece na composicao — os pesos sao iguais —, entao
                      tem de ser dita aqui, com os numeros dos dois lado a lado. */}
                  {d.volTarget && (
                    <div style={{ display: "flex", gap: 10, alignItems: "baseline" }}>
                      <b style={{
                        fontFamily: "var(--mono)", color: "var(--gold)", fontSize: 13,
                        minWidth: 34, textAlign: "right", flexShrink: 0,
                      }}>+</b>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: 12.5, color: "var(--tx1)" }}>
                          Teto de volatilidade em {(d.volTarget.alvo * 100).toFixed(0)}% ao ano{" "}
                          <span style={{ color: "var(--tx3)", fontSize: 12 }}>
                            — overlay proprietário Harpian
                          </span>
                        </div>
                        <div style={{
                          fontSize: 12, color: "var(--tx2)", lineHeight: 1.45,
                          ...
                          {comparativoOverlay ? (
                            <>
                              A composição acima é <b>idêntica</b> à do {comparativoOverlay.par.rotuloCurto}.
                              O que muda é só isto: toda semana o sistema mede a volatilidade das últimas{" "}
                              {d.volTarget.lookback} sessões e, se ela passou do teto, reduz a exposição
                              e põe a diferença em caixa. Na janela validada, a maior queda vai de{" "}
                              <b style={{ fontFamily: "var(--mono)" }}>
                                {pct(comparativoOverlay.sem.maxDD, 1)}
                              </b>{" "}para{" "}
                              <b style={{ fontFamily: "var(--mono)", color: "var(--green)" }}>
                                {pct(comparativoOverlay.com.maxDD, 1)}
                              </b>{" "}e o Sharpe <b>sobe</b> de{" "}
                              <b style={{ fontFamily: "var(--mono)" }}>
                                {comparativoOverlay.sem.sharpe.toFixed(2)}
                              </b>{" "}para{" "}
                              <b style={{ fontFamily: "var(--mono)", color: "var(--green)" }}>
                                {comparativoOverlay.com.sharpe.toFixed(2)}
                              </b>. Menos queda sem custar retorno ajustado a risco.
                            </>
                          ) : (
                            <>
                              A composição acima é a mesma do SET balanceado da linha. O que muda é o
                              overlay: toda semana o sistema mede a volatilidade das últimas{" "}
                              {d.volTarget.lookback} sessões e, se passou do teto, reduz a exposição e
                              põe a diferença em caixa. Nunca alavanca.
                            </>
                          )}
                        </div>
                      </div>
