...
// Individual execution popup — used both by the "Execute" button on Mission Control
// and by the action badge (INCREASE/REDUCE/...) on the asset chart. Shows the
// portfolio, the order size, and lets the user choose market vs set price before
// confirming — only then does it become a ticket sent to IBKR (Air Gap: a human always approves).
export default function ExecuteOrderModal({ ticketId, onClose, onExecuted }: {
  ...
  async function confirmar() {
    ...
    if (orderType === "limite" && (!limitPrice || isNaN(Number(limitPrice)) || Number(limitPrice) <= 0)) {
      dialog.notify("Enter a valid limit price.", "error");
      return;
    }
    setSending(true);
    try {
      ...
      dialog.notify(
        `${ticket.ticker} sent to IBKR — ${orderType === "mercado" ? "at market" : `limit $${limitPrice}`}.`,
        "success"
      );
      onExecuted(updated);
      onClose();
    } catch (e) {
      dialog.notify("Error executing order: " + String((e as Error).message || e), "error");
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(4,10,20,.62)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300 }}>
      <div onClick={(e) => e.stopPropagation()} className="card" style={{ width: 440, maxWidth: "92vw", padding: 20 }}>
        {loading ? (
          <div className="ph" style={{ padding: 24 }}>Loading ticket…</div>
        ) : !ticket ? (
          <div className="ph" style={{ padding: 24 }}><b>Ticket not found</b></div>
        ) : (
          <>
            <div className="flex between" style={{ marginBottom: 4, alignItems: "center" }}>
              <div style={{ fontSize: 17, fontWeight: 800 }}>Execute {ticket.ticker}</div>
              <span className={`side ${ticket.side}`}>{ticket.side === "buy" ? "BUY" : "SELL"}</span>
            </div>
            <div className="c-mut" style={{ fontSize: 12, marginBottom: 14 }}>{ticket.nome || ticket.ticker} · {ticket.motivo}</div>

            <div className="kv"><span className="c-mut">Portfolio</span><span className="v">{PORT_LABEL[ticket.portfolio_id] || ticket.portfolio_id}</span></div>
            <div className="kv"><span className="c-mut">Quantity</span><span className="v">{ticket.quantidade.toLocaleString("en-US")} units</span></div>
            <div className="kv"><span className="c-mut">Estimated price</span><span className="v">${precoEst.toFixed(2)}</span></div>
            <div className="kv"><span className="c-mut">Total value</span><span className="v" style={{ color: "var(--gold)", fontWeight: 700 }}>${ticket.valor_usd.toLocaleString("en-US")}</span></div>
            {ticket.troca_para && <div className="kv"><span className="c-mut">Swap to</span><span className="v">{ticket.troca_para}</span></div>}
            {jaExecutado && (
            ...
            {!jaExecutado && (
              <>
                <div style={{ marginTop: 16, marginBottom: 6, fontSize: 11, color: "var(--tx3)", textTransform: "uppercase", letterSpacing: .8 }}>Order type</div>
                ...
                {orderType === "limite" && (
                  <input
                    ...
                    value={limitPrice} onChange={(e) => setLimitPrice(e.target.value)} placeholder="Limit price ($)"
                  />
                )}
              </>
            )}
            ...
            <div style={{ marginTop: 18, display: "flex", gap: 8, justifyContent: "flex-end" }}>
              ...
              {!jaExecutado && (
                <button className="btn" onClick={confirmar} disabled={sending}>
                  {sending ? "Sending…" : "Confirm and send to IBKR"}
                </button>
              )}
            </div>
          </>
